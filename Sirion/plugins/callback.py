...
from Sirion.utils.inline import (
...
@languageCB
async def unban_ass(cli, CallbackQuery, _):
    callback_data = CallbackQuery.data.strip()
    callback_request = callback_data.split(None, 1)[1]
    ...
    if umm.can_restrict_members:
        try:
            ...
            return await CallbackQuery.answer(_["call_12"], show_alert=True)
        return await CallbackQuery.edit_message_text(_["call_13"].format(userbot.one.name, CallbackQuery.from_user.mention))
    else:
        return await CallbackQuery.answer(_["call_14"], show_alert=True)
