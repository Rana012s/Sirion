...
from Sirion.utils.database import get_global_tops, get_particulars, get_userss
from Sirion.utils.inline.playlist import botplaylist_markup, failed_top_markup, top_play_markup
...
async def server_to_play(client, CallbackQuery, _):
    ...
    if not stats:
        return await mystic.edit_message_text(_["tracks_2"].format(what), reply_markup=upl)
    ...
    def get_stats():
        ...
        if not results:
            return mystic.edit_message_text(_["tracks_2"].format(what), reply_markup=upl)
        details = []
        limit = 0
